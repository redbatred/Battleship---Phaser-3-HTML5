...
export class Player2SetupScene extends BaseScene {
    ...
    private resetShipCounter(): void {
        ...
        if (this.registry.has('player2Grid')) {
            this.registry.remove('player2Grid');
        }
        if (this.registry.has('player2Ships')) {
            this.registry.remove('player2Ships');
        }
        
        console.log('Player2 ship counter and placement state reset');
    }
    ...
    create(data?: any) {
        ...
        console.log('Player2SetupScene: create');
        ...
        this.playerGrid = new Grid(this, screenWidth/2, screenHeight/2, 10, 10, 'Player 2 Fleet');
        ...
        this.add.text(
            screenWidth/2, 
            50, 
            'PLAYER 2: Place your ships on the grid', 
            {
                fontFamily: 'Impact',
                fontSize: '28px',
                color: '#ffffff',
        ...
        forceButton.on('pointerdown', () => {
            console.log('Forcing continue to BattleScene');
            
            // Force all ships to be placed
            this.currentShipIndex = this.shipConfigs.length;
            
            // Store player 2's data in the registry
            this.registry.set('player2Grid', this.playerGrid);
            this.registry.set('player2Ships', this.playerShips);
    ...
    private createOceanBackground(): void {
        ...
        waterLayer1.setTint(0xcc0000, 0xcc0000, 0xcc0000, 0xcc0000); // Reddish tint for player 2
    ...
    private createRotateButton(): void {
        ...
        // Create button background - reddish for player 2
        const buttonBg = this.add.graphics();
        buttonBg.fillStyle(0x761a1a, 1);
        buttonBg.fillCircle(0, 0, buttonWidth/2);
        
        // Add highlight
        buttonBg.fillStyle(0xb92929, 0.7);
        buttonBg.fillCircle(0, -5, buttonWidth/2 - 10);
        
        // Add border
        buttonBg.lineStyle(3, 0xdb3434);
        ...
        this.rotateButton.on('pointerover', () => {
            ...
            buttonBg.fillStyle(0xb92929, 1);
            buttonBg.fillCircle(0, 0, buttonWidth/2);
            
            // Brighter highlight
            buttonBg.fillStyle(0xdb3434, 0.7);
            buttonBg.fillCircle(0, -5, buttonWidth/2 - 10);
            
            // Brighter border
            buttonBg.lineStyle(3, 0xf96767);
            buttonBg.strokeCircle(0, 0, buttonWidth/2);
    ...
    private createConfirmButton(): void {
        ...
        this.confirmButton.on('pointerdown', () => {
            ...
            if (actualPlacedCount >= this.shipConfigs.length) {
                console.log('Player2SetupScene: Transitioning to BattleScene');
    ...
    private createRandomButton(): void {
        ...
        // Create button background with naval theme - reddish for player 2
        const buttonBg = this.add.graphics();
        buttonBg.fillStyle(0x5a1a1a, 1); // Darker red for contrast
        buttonBg.fillRoundedRect(-buttonWidth/2, -buttonHeight/2, buttonWidth, buttonHeight, 10);
        
        // Add metallic highlight
        buttonBg.fillStyle(0xb92929, 0.5);
        buttonBg.fillRoundedRect(-buttonWidth/2, -buttonHeight/2, buttonWidth, buttonHeight/2, 10);
        ...
        waveGraphics.lineStyle(2, 0xf96767, 0.4);
    ...
    private placeShipsRandomly(): void {
        ...
        // Update the ship counter safely
        if (this.scene.isActive()) {
            try {
                this.updateShipCounter();
            } catch (error) {
                console.warn('Error updating ship counter after random placement:', error);
            }
        }
    ...
    private createShipsForPlacement(): void {
        ...
        const startX = this.cameras.main.width - 100;
        const startY = 150;
        const spacing = 60;
            ...
            if (this.playerGrid.canPlaceShip(ship.getType(), row, col, isHorizontal)) {
                ...
                if (this.scene.isActive()) {
                    try {
                        ...
                        console.warn('Error updating ship counter after ship drop:', error);
                    }
                }
        ...
        if (gridPos) {
            ...
            if (this.scene.isActive()) {
                try {
                    ...
                    console.warn('Error updating ship counter after ship pickup:', error);
                }
            }
        }
